package myPackage;

public class Aggregat {

	public static void main(String[] args) {
		System.out.println("Hallo");
		System.out.println("zweites Commit");
		System.out.println("drittes Commit");

		System.out.println("Commit von schwarzem eclipse");
		System.out.println("Commit von schwarzem eclipse 02");
		
		System.out.println("Kalles erstes Commit");
		System.out.println("Kalles drittes Commit");
		
		System.out.println("Commit von schwarzem eclipse 03");
		System.out.println("Kalles viertes Commit");

		System.out.println("Commit von schwarzem eclipse 05");

		System.out.println("Kalles fuenftes Commit");
		
		System.out.println("Commit von schwarzem eclipse 06");
		
		System.out.println("Kalles Commit Nr 6");
		
		System.out.println("Weisses Commit Nr 001");
		
		System.out.println("Weisses Commit Nr 002");
		
		System.out.println("Weisses Commit Nr 003");
		
		System.out.println("Weisses Commit Nr 004");
		
		System.out.println("Weisses Commit Nr 005");
		
		System.out.println("Weisses Commit Nr 006");
		
		System.out.println("Weisses Commit Nr 007");
		
		System.out.println("Weisses Commit Nr 008");
		
		
				
	}

}
